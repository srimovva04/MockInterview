...
import subprocess
import tempfile
from gemini_resume_builder_helper import refine_bullet_points
...
#     technologies_tex = "\n".join(f"\\item {tech}" for tech in data['technologies'])
#     skills_tex = f"\\item {data['skills']}" if data['skills'] else ""
...
def generate_latex(data):
    ...
    experience_tex = ""
    for exp in data['experience']:
        refined = refine_bullet_points(exp.get('bullets', []), section_name="Experience")
        experience_tex += f"""
\\begin{{cvsubsection}}{{{exp['title']}}}{{{exp['company']}}}{{{exp['dates']}}}
{itemize(refined)}
\\end{{cvsubsection}}
"""
    ...
    education_tex = ""
    for edu in data['education']:
        refined = refine_bullet_points(edu.get('details', []), section_name="Education")
        education_tex += f"""
\\begin{{cvsubsection}}{{{edu['location']}}}{{{edu['institution']}}}{{{edu['dates']}}}
{itemize(refined)}
\\end{{cvsubsection}}
"""
    ...
    projects_tex = ""
    for proj in data['projects']:
        refined = refine_bullet_points(proj.get('bullets', []), section_name="Project")
        projects_tex += f"""
\\begin{{cvsubsection}}{{{proj['name']}}}{{}}{{{proj['year']}}}
{itemize(refined)}
\\end{{cvsubsection}}
