...
const InterviewTable = () => {
  ...
  const fetchCombinedData = async () => {
    ...
    console.log('Job readiness data:', jobData);

    if (jobError) {
      console.error('Error fetching job readiness data:', jobError);
    }

    const formattedInterviews = (interviewData || []).map((item) => ({
      id: item.id,
    ...
    const formattedJobs = (jobData || []).map((item) => ({
      id: item.id,
      interview: 'Job Readiness Assessment',
    ...
    const allData = [...formattedInterviews, ...formattedJobs].sort(
