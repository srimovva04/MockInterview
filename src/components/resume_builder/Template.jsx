...
export const ClassicTemplate = ({ resumeData }) => {
  return (
    <div className="bg-white text-black p-4 rounded-lg text-sm font-serif leading-tight max-w-full"
      ...
      {resumeData.employment?.length > 0 && (
        <Section title="Employment" class="section-container">
      ...
      {resumeData.education?.length > 0 && (
        <Section title="Education" class="section-container">
      ...
      {resumeData.projects?.length > 0 && (
        <Section title="Technical Experience" class="section-container">
      ...
      {resumeData.additionalExperience?.filter(exp => exp.trim()).length > 0 && (
        <Section title="Additional Experience & Awards" class="section-container">
      ...
      {(resumeData.skills?.languages?.length > 0 || resumeData.skills?.tools?.length > 0) && (
        <Section title="Languages and Technologies" class="section-container">
...
export const ResumePreview = ({ resumeData }) => {
  return (
    <div 
      className="bg-white text-[#374151] p-4  rounded-lg text-sm font-sans leading-relaxed max-w-full"
      style={{ 
        minHeight: '11in',
        width: '8.5in',
        maxWidth: '100%',
    ...
    >
      ...
      {/* Work Experience */}
      {resumeData.employment.length > 0 && (
        <Section title="Work Experience" class="section-container">
      ...
      {/* Skills */}
      {(resumeData.skills.languages.length > 0 || resumeData.skills.tools.length > 0) && (
        <Section title="Skills & Technologies" class="section-container">
          {resumeData.skills.languages.length > 0 && (
            <p className="mb-1">
              <strong>Languages:</strong> {resumeData.skills.languages.join(", ")}
            </p>
...
const Section = ({ title, children, className }) => (
  // Added "break-inside-avoid" to prevent splitting across pages
  <div className={`mb-3 break-inside-avoid ${className || ''}`}>
