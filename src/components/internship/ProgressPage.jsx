...
import { Check, ArrowRight, Trophy, ChevronDown, AlertCircle } from 'lucide-react';
import { fetchSimulations, fetchTasksForSimulation } from '../utils/simulations';
import { UserAuth } from '../AuthContext'; // adjust path if needed
  ...
  const mergeTasksWithProgress = (tasks, progressData, simulationId) => {
    return tasks.map(task => {
      const progress = progressData.find(p => 
        p.task_id === task.id && p.simulation_id === simulationId
      );
  ...
  useEffect(() => {
    const loadSimulationsWithProgress = async () => {
      ...
      try {
        ...
        const userSimulationIds = [...new Set(allUserProgress.map(p => p.simulation_id))];
        console.log('User has progress in simulations:', userSimulationIds);

        // Filter simulations to only show those the user has started
        const userSimulations = simData.filter(sim => userSimulationIds.includes(sim.id));
        ...
        const simulationsWithProgress = await Promise.all(
          userSimulations.map(async (sim) => {
            try {
              ...
              const simProgressData = allUserProgress.filter(p => p.simulation_id === sim.id);
              ...
              const tasksWithProgress = mergeTasksWithProgress(userTasks, simProgressData, sim.id);

              // Calculate progress statistics
              const completedTasks = tasksWithProgress.filter(t => t.status === 'completed').length;
              const inProgressTasks = tasksWithProgress.filter(t => t.status === 'in_progress').length;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <p className="text-gray-600">Please sign in to view your simulation progress.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-800 mb-2">No Progress Yet</h3>
            <p className="text-gray-600 mb-6">
              You haven't started any simulations yet. Begin your first simulation to see your progress here.
            </p>
            <button 
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className={`px-3 py-1 rounded-full text-xs font-semibold ${
                        ...
                        {sim.status === 'not_started' ? 'Not Started' : 
                         sim.status === 'in_progress' ? 'In Progress' : 
                         sim.status === 'error' ? 'Error' : 'Completed'}
                      </div>
                      ...
                      {sim.status !== 'error' ? (
                        <>
                            ...
                            {sim.tasks.length > 0 ? (
                              <div className="border border-gray-200 rounded-xl overflow-hidden divide-y divide-gray-100">
                                {sim.tasks.map((task, index) => (
                                  <TaskItem key={`${sim.id}-${task.id}`} task={task} number={index + 1} />
                                ))}
                              </div>
                            ) : (
                              <p className="text-gray-500 text-center py-4">No tasks started yet</p>
                            )}
                          </div>
                          ...
                          <div className="flex gap-3">
                            ...
                            >
                              {sim.status === 'completed' ? 'View Completed Tasks' :
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-red-600 font-medium">Error loading simulation data</p>
